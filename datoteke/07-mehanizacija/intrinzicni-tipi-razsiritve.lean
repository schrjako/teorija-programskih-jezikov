...
theorem StepDeterministic : Step M M1 → Step M M2 → M1 = M2 := by
    ...
    case app_step1 MtoM' =>
        ...
        apply MtoM' -- ih za step1
        assumption
        -- kaj pa če je step2 app_step2? (ja ne bo šlo)
        simp
        rename_i h1 h2
        exfalso
        ...
        exists m2
        rcases m4
        -- Nekoliko bolj zapleten dokaz (če mimamo dokaza da se vrednosti ne izvajajo)
        -- have kk := canonicalLam2 h1 -- vemo da je v tem primeru ta vrednost in tipa funkcija, pol pa ne bo šlo, ker ne more naprej v temu kar imamo v indukciji
        -- obtain ⟨body, body_eq⟩ := kk
        -- rw [body_eq] at m4
        -- rcases m4
        -- rcases m4 -- ne morermo imeti substitucije - isti argument
    case app_step2 V NtoN' =>
        rename_i vv a b c
        rcases step2
        exfalso
